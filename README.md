...
- **One segmented progress bar for the whole class, live.** The bar is divided into a segment per activity. Each segment shows the activity's name and duration in its middle, and the boundary times between activities sit above the bar. The last activity always absorbs whatever the earlier ones ran under or over, so the bar keeps spanning the full class: finish an activity early and everything after it shifts left (the last segment grows); run long and everything shifts right (the last segment shrinks). The "First/Next: ..." hint only appears as a fallback when a segment is too narrow to show its own label. The current segment fills as it runs, turns amber for the final stretch, and blinks when time is up (with an optional chime) until you press **Next Activity** — which you can also press early to move on ahead of schedule.
