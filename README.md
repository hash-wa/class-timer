...
- **Class window and time left.** Once a class starts, its actual start–end window appears right next to the class name, and a line below shows how much class time is left overall — with the same small-seconds styling as the clock — turning red with a "+over" once it runs past its own end.
